...
// aer-0118: hard ceiling on concurrent slice slots. Bounds the scratch key
// space and the pool/output memory multiplier; AER_TN_STREAMS is clamped here.
static const int kMaxStreamSlots = 8;
...
struct StageIdx {
  ...
  void fill(const std::vector<int64_t> &extents,
    ...
    // aer-0133: measure the run. Walk modes while each stride equals the
    // running product of the extents before it -- the definition of natural
    // packed order.
    run = 1;
    for (int k = 0; k < rank; ++k) {
      if (str[k] != run)
...
// distribution that says whether the vectorised path is reaching most of the
// volume or almost none of it, which is what decides whether further kernel
// work (LDS tiling) is worth attempting after this patch is measured.
// Process-local, written only under tn_profile().
inline uint64_t *pack_run_hist() {
  static uint64_t h[8] = {0, 0, 0, 0, 0, 0, 0, 0};
  return h;
}
inline void pack_note_run(int64_t run, uint64_t n) {
  int b = 0;
  while (b < 7 && (int64_t{1} << (b + 1)) <= run)
    ++b;
  pack_run_hist()[b] += n;
}
...
void TensorNetContractor_HipTensor<data_t>::setup_pool_and_cache(int device_idx,
  ...
  for (size_t step = 0; step < num_steps; step++) {
    ...
    if (step == num_steps - 1) {
      ...
      if (tn_order_prop() && cs >= 0 && consumer_count[result_idx] == 1 &&
        ...
        if (!shared_at_consumer.empty()) {
          ...
          // scatter-only route (aer-0117) for no benefit downstream. The
...
#ifdef AER_HIPBLAS
    {
      ...
      // comparable on a replayed plan.
      {
        ...
        //
        // aer-0117: c_ok is NOT required here. It was, and that silently
        ...
        if (strides_a.empty() && strides_b.empty() &&
          ...
          gemm_steps_routed_++;
          // aer-0117: operands are clean but C is not in [M][N] order (order
    ...
    // linear pair otherwise. Exactly the pairs the dispatch would have passed.
    {
      ...
      // aer-0133: census the run distribution here, where the shapes are
      // final and the cost is one classification-time bucket increment per
      // role -- not per dispatch. Weighted by elements, not by step count,
      // because the question is what fraction of the VOLUME the vectorised
      // path reaches. Counted once per plan step; the slice loop multiplies
      // every step by the local slice count, so read the histogram as a
      // distribution, not as a total.
      if (tn_profile()) {
        pack_note_run(sp.sidx_a.run, sp.sn_a);
        pack_note_run(sp.sidx_b.run, sp.sn_b);
        pack_note_run(sp.sidx_c.run, sp.sn_c);
      }
    }
  ...
  {
    // aer-0118: reserve one pool region per stream slot. tn_streams()==1
    // reserves exactly one, i.e. the pre-0119 allocation.
    int ps = tn_streams();
    if (ps > kMaxStreamSlots) ps = kMaxStreamSlots;
    if (ps < 1) ps = 1;
    dev.pool().allocate(dev.device_id(), ps);
  }
  if (tn_profile())
...
void TensorNetContractor_HipTensor<data_t>::prof_report(
  ...
  // dominates, the remaining win needs LDS tiling rather than wider groups.
  std::string run_hist;
  {
    std::ostringstream rh;
    const uint64_t *h = pack_run_hist();
    for (int b = 0; b < 8; ++b) {
      if (b)
        rh << ',';
      rh << h[b];
...
void TensorNetContractor_HipTensor<data_t>::contract_all() {
  ...
  // stream, a hipBLAS handle, a full copy of the intermediate pool layout, a
  // scratch region and an output region, so slices in flight share nothing
  // writable. Inputs are the one exception and are safe: project_slice points
  // at the read-only input slab.
  //
  // The memory cost is real and is why this is opt-in: N copies of the
  // intermediate pool, which is the term slicing exists to shrink. Size it
  // against measured headroom, not by assumption.
  int nslots = tn_streams();
  if (nslots > kMaxStreamSlots) nslots = kMaxStreamSlots;
  ...
  for (int idev = 0; idev < num_devices_used_; idev++) {
    ...
    hipSetDevice(gpu_mgr_.device(idev).device_id());
    // More slots than slices on this device is pure memory waste.
    int dev_slots = nslots;
    const uint_t dev_count = dev_slice_end - dev_slice_begin;
    if ((uint_t)dev_slots > dev_count) dev_slots = (int)(dev_count ? dev_count : 1);
    if (dev_slots > 1) {
      gpu_mgr_.device(idev).ensure_slots(dev_slots);
      gpu_mgr_.device(idev).resize_output_slots(out_size_, dev_slots);
    }
    ...
    for (uint_t s = dev_slice_begin; s < dev_slice_end; s++) {
      const int slot =
          (dev_slots > 1) ? (int)((s - dev_slice_begin) % (uint_t)dev_slots) : 0;
      contract_single_slice(s, idev, slot);
    ...
    // aer-0118: join every slot before the partials are summed -- the
    // reduction reads regions other streams are still writing otherwise --
    // then fold slots 1..n-1 into region 0, which is what the cross-device
    // and cross-rank reductions consume.
    if (dev_slots > 1) {
